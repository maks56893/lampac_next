(function () {
    ...
    // Все обращения идут только на сервер Lampac ({localhost}/syno_dlna/...). Адрес самого
    // DLNA-сервера клиенту неизвестен и не нужен - он задан в init.conf модуля SynoDLNA.
    // Единственное, что вводит пользователь - токен доступа (см. настройки ниже).

    // ВНИМАНИЕ: это НЕ авторизация самого SynoDLNA - она остаётся только по X-Syno-Token
    // (см. Controller.Authorize на сервере, который не смотрит на account_email/uid/group).
    // Это отдельный, независимый слой: если на сервере включен accsdb, каждый запрос ДО
    // контроллера проходит через его middleware (Core/Middlewares/Accsdb.cs), которому нужен
    // account_email+uid зарегистрированного устройства (или token) в query - иначе он отбивает
    // запрос сам, контроллер SynoDLNA даже не вызывается. Параметры добавляются здесь только
    // чтобы пройти этот слой; не убирать и не смешивать с проверкой токена модуля.
    ...
    function request(path, data, success, error, timeout) {
      $.ajax({
        url: account('{localhost}/syno_dlna/' + path),
        type: 'GET',
        data: data || {},
        dataType: 'json',
        timeout: timeout || 15000,
        headers: {
          'X-Syno-Token': Lampa.Storage.get('syno_dlna_token', '') || ''
        },
    ...
    function Component(object) {
      ...
      this.create = function () {
        ...
        tree = {
          tree: [{ title: '/', id: '', isRootFolder: true }]
        };
      ...
      this.showEmpty = function (xhr) {
        var text = Lampa.Lang.translate('synology_dlna_client_error');
        if (xhr && xhr.status === 401) text = Lampa.Lang.translate('synology_dlna_client_no_token');
        else if (xhr && xhr.status === 403) text = Lampa.Lang.translate('synology_dlna_client_disabled');
      ...
      this.drawFolder = function (folders, files) {
        var _this2 = this;

        scroll.clear();
        scroll.reset();

        console.log('SynoDLNA', 'files', files.length, 'with subtitles', files.filter(function (f) {
          return f.subtitles && f.subtitles.length;
        }).length);
        ...
        if (files.length) {
          ...
          files.forEach(function (element) {
            ...
            add += element.subtitles && element.subtitles.length ? 'SUB ' : '';
            ...
            item.on('hover:enter', function () {
              if (element.type === 'object.item.imageItem.photo') {
                ...
                // create image tag at fullscreen and wait back or esc for close
      ...
      this.startPlay = function (element, subtitles) {
        var video = {
          title: element.title,
          url: element.url
        };
        if (subtitles && subtitles.length) {
          video.subtitles = subtitles.map(function (sub) {
            return {
              method: 'link',
              label: sub.label,
              url: sub.url
          ...
          console.log('SynoDLNA', 'subtitles', video.subtitles);
        }
        Lampa.Player.play(video);
        Lampa.Player.playlist([video]);
      };

      this.playFile = function (element) {
        var _this = this;

        if (element.type !== 'object.item.videoItem' || Lampa.Storage.get('synology_dlna_subtitles', true) === false) {
          return this.startPlay(element, []);
        }
        if (element.subtitles && element.subtitles.length) {
          return this.startPlay(element, element.subtitles);
        }
        if (element.subtitles_checked || !element.id) {
          return this.startPlay(element, []);
        }

        request('subtitles', { id: element.id }, function (resp) {
          element.subtitles = (resp && resp.subtitles) || [];
          element.subtitles_checked = true;
          _this.startPlay(element, element.subtitles);
        }, function () {
          element.subtitles = [];
          element.subtitles_checked = true;
          _this.startPlay(element, []);
        }, 8000);
      };
      ...
      this.drawHead = function () {
        ...
        icon += '<span>' + Lampa.Lang.translate('title_syno_dlna_head') + '</span>';
      ...
      this.browse = function (object_id, flag, success, error) {
        request('browse', { id: object_id, flag: flag }, function (resp) {
          success((resp && resp.folders) || [], (resp && resp.files) || []);
        }, error);
      };

      this.displayFolder = function (first) {
        var _this3 = this;

        var folder = tree.tree[tree.tree.length - 1];
        this.drawLoading(Lampa.Lang.translate('loading'));

        this.browse(folder.id, 'BrowseDirectChildren', function (folders, files) {
          _this3.drawFolder(folders, files);
        }, function (xhr) {
          if (first) _this3.showEmpty(xhr);
          else _this3.showError(xhr);
        });
      };
    ...
    function startPlugin() {
      ...
      Lampa.Lang.add({
        synology_dlna_client_no_token: {
          ru: 'Введите токен доступа в настройках SynoDLNA',
          en: 'Enter the SynoDLNA access token in the settings'
        },
        synology_dlna_client_disabled: {
          ru: 'Модуль SynoDLNA отключен на сервере',
          en: 'The SynoDLNA module is disabled on the server'
        },
        synology_dlna_client_error: {
          ru: 'Не удалось получить данные от DLNA сервера',
          en: 'Failed to get data from the DLNA server'
        },
        title_syno_dlna_head: {
          ru: 'DLNA',
          en: 'DLNA'
        }
      });
      ...
      function add() {
        ...
        Lampa.SettingsApi.addParam({
          ...
          field: {
            name: 'Токен доступа',
            description: 'Выдаётся администратором сервера'
          }
        });
        Lampa.SettingsApi.addParam({
          component: 'synology_dlna_client_config',
          ...
          onChange: function () {
            request('ping', {}, function () {
              Lampa.Noty.show('Токен верный, соединение установлено');
            }, function (xhr) {
              if (xhr && xhr.status === 401) Lampa.Noty.show('Неверный токен доступа');
              else if (xhr && xhr.status === 403) Lampa.Noty.show('Модуль SynoDLNA отключен на сервере');
              else Lampa.Noty.show('Не удалось выполнить проверку, проверьте соединение с сервером');
            });
          }
        });
        Lampa.SettingsApi.addParam({
          component: 'synology_dlna_client_config',
          param: {
            name: 'synology_dlna_subtitles',
            type: 'trigger',
            default: true
          },
          field: {
            name: 'Внешние субтитры',
            description: 'Подключать внешние субтитры с DLNA сервера к видео'
          }
        });
