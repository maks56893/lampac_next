...
public class SynoDLNAController : BaseController
{
    ...
    /// Разрешает отдать LAN-клиенту прямую ссылку на NAS - но только когда включён
    /// directLocalIp, а он по умолчанию выключен.
    ///
    /// Прямая ссылка работает лишь если сам DLNA-сервер отдаёт заголовки CORS. Synology на
    /// старом pupnp SDK их не отдаёт, и браузер блокирует запрос: у субтитров - потому что
    /// Лампа качает их через XHR, у видео - потому что плеер Лампы запрашивает поток в
    /// режиме с проверкой CORS (проверено на практике: "Access to video at ... has been
    /// blocked by CORS policy"). Ровно ради этого случая раньше и держали внешний
    /// CORS-прокси; отдача через /proxy снимает проблему для всех типов ресурсов.
    ///
    /// Для субтитров параметр обязан быть false всегда, независимо от конфига.
    /// </param>
